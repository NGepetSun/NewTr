...
function normalize(raw){
  ...
  let teams=Array.from({length:8},(_,i)=>({
    ...
    side:i<4?'idp':'ime',
    players:Array.from({length:5},(_,p)=>x.teams?.[i]?.players?.[p]??d.teams[i].players[p])
  }));
  return {teams,scores:x.scores||{},winners:x.winners||{},live:{...d.live,...(x.live||{})}};
}
...
function matches(){
  ...
  const q=[['q1','QUARTER FINAL 01',t[0],t[1]],['q2','QUARTER FINAL 02',t[2],t[3]],['q3','QUARTER FINAL 03',t[4],t[5]],['q4','QUARTER FINAL 04',t[6],t[7]]];
  const s=[['s1','SEMI FINAL 01',winner('q1',t[0],t[1]),winner('q2',t[2],t[3])],['s2','SEMI FINAL 02',winner('q3',t[4],t[5]),winner('q4',t[6],t[7])]];
...
function renderRoster(){const el=document.querySelector('#rosterGrid');if(!el)return;el.innerHTML=state.teams.map((t,i)=>`<article class="roster-card ${t.side}"><div class="roster-head"><div class="mini-crest ${t.side}">${String.fromCharCode(65+i%4)}</div><div><span>MAPENDOS ${t.side.toUpperCase()} · TEAM ${String.fromCharCode(65+i%4)}</span><h3>${esc(t.name)}</h3></div></div><div class="players">${t.players.map((p,j)=>`<div><span class="num">0${j+1}</span><b>${esc(p)}</b></div>`).join('')}</div></article>`).join('')}
...
function addParticipants(names,side){
  ...
  const pools={idp,ime};
  ['idp','ime'].forEach(f=>{const arr=pools[f];for(let team=0;team<4;team++)state.teams[team+(f==='ime'?4:0)].players=Array.from({length:5},(_,j)=>arr[team*5+j]||'TBD')});
...
function shufflePlayers(){
  ...
  for(let team=0;team<4;team++){state.teams[team].players=idp.slice(team*5,team*5+5);state.teams[team+4].players=ime.slice(team*5,team*5+5)}
  state.winners={};state.scores={};save();renderAll();renderParticipantList();
  const note=document.querySelector('#shuffleNote');if(note){note.textContent='Peserta berhasil dikocok. IDP tetap bersama IDP, IME tetap bersama IME.';note.classList.add('success')}
}
...
function renderAll(){renderBracket();renderRoster();renderAdminTeams();renderResults();renderStats();renderLive()}
...
function setupAdmin(){
  ...
  const demo=document.querySelector('#fillDemo');if(demo)demo.onclick=()=>{state=clone(DEFAULT);save();renderAll();renderParticipantList()};
...
setupNav();setupRosterModal();setupAdmin();renderAll();renderParticipantList();
